...
| **maxRetryCount** | **Integer** | Maximum number of retries that should be attempted to try and connect a customer first callback to a customer when the initial callback attempt did not connect. |  [optional] |
| **retryDelaySeconds** | **Integer** | Delay in seconds between each retry of a customer first callback. |  [optional] |
